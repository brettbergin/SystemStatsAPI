...
def register():
    ...
    user_exists = User.query.filter_by(username=username).one_or_none() \
        is not None or User.query.filter_by(email=email).one_or_none() is not None
...
def refresh():
    ...
    access_token = create_access_token(identity=identity, additional_claims=info_claims, fresh=False)
    refresh_token = create_refresh_token(identity=identity, additional_claims=info_claims)
    return jsonify(access_token=access_token, refresh_token=refresh_token)
